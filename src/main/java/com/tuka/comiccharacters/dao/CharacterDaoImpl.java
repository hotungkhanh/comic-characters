...
public class CharacterDaoImpl extends AbstractJpaDao<ComicCharacter> {
    ...
    public ComicCharacter findByIdWithDetails(Long id) {
        try (EntityManager em = getEntityManager()) {
            return em.createQuery("SELECT DISTINCT c FROM ComicCharacter c " +
                            "LEFT JOIN FETCH c.creators " +
                            "LEFT JOIN FETCH c.issues i " +
                            "LEFT JOIN FETCH i.series " +
                            "LEFT JOIN FETCH c.firstAppearance " +
                            "LEFT JOIN FETCH c.publisher " +
                            "WHERE c.id = :id", ComicCharacter.class)
                    .setParameter("id", id)
                    .getResultStream()
                    .findFirst()
                    .orElse(null);
